...
class SantaNumberPicker
    ...
    constructor(
        ...
        init {
            ...
            setAdapter(adapter)
            addOnScrollListener(
                SantaNumberPickerSnapScrollListener(this) { snapPosition ->
        ...
        private fun initAttribute(attrs: AttributeSet?) {
            ...
            if (typeArray.hasValue(R.styleable.SantaNumberPicker_minValue)) {
                startNumber =
                    typeArray.getInt(R.styleable.SantaNumberPicker_minValue, DEFAULT_START_NUMBER)
            }
        ...
        private fun setTextStyleByPosition(
            textView: AppCompatTextView,
            distanceFromCenterPosition: Int,
        ) {
            if (distanceFromCenterPosition == 0) {
                ...
                textView.setTextColorById(R.color.dark_gray)
            }
        }

        private fun setNonCenterTextStyle(
            textView: AppCompatTextView,
            distanceFromCenterPosition: Int,
        ) {
        ...
        private inner class SantaNumberPickerAdapter :
            Adapter<SantaNumberPickerAdapter.SantaNumberPickerViewHolder>() {
            var currentCenterPosition = initialCenterPosition + numberOfEmptySpace

            override fun onCreateViewHolder(
                parent: ViewGroup,
                viewType: Int,
            ): SantaNumberPickerViewHolder {
                return SantaNumberPickerViewHolder(getTextView(parent.context))
            }

            override fun onBindViewHolder(
                holder: SantaNumberPickerViewHolder,
                position: Int,
            ) {
                val number =
                    if (isInNumberRange(position)) {
